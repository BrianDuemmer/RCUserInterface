...
public class Foobar 
{
	...
	public static double addSongTime = 5;
	...
	private void doUpdate()
	{
		...
		if(isPlaying) // queue only proceeds if playing
		{
			...
			if(currSongRemaining < 0) // we are onto the next song
			{
				currSong = queue.poll();
				currSongRemaining += currSong.getLength();
				//				totalQueueLength += currSong.getLength();
//				System.out.println("Foobar moved to next song");
			}
...
//		System.out.println(String.format("csr: %.04f  tot: %.04f isP: %B", currSongRemaining, totalQueueLength, isPlaying));
	}
	...
	public void execFoobarCmd(String foobarCmd)
	{
		...
		try {
			...
			/** That didn't work... it just hung there on those calls. Good thing we don't ned the output. */
	...
	public void addToPlaybackQueue(QueueEntry song)
	{
		if(song.getSong().getSongID().isEmpty()) // bad song
			...
			String songPath = Dystrack.playlistRoot +"/"+ song.getSong().getSongID();
	...
	private void startUpdaterFresh()
	{
		...
		// and start
		updateSrv.scheduleAtFixedRate(() -> { doUpdate(); }, 0, Dystrack.simulatorUpdateMillis, TimeUnit.MILLISECONDS);
	}
