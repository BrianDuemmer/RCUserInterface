...
public class QueueEntry
{
	...
	public static ObservableList<QueueEntry> getForwardQueue()
	{
		...
		try 
		{
			...
			ResultSet rs = Dystrack.db.execRaw("SELECT * FROM " +RCTables.forwardQueueTable.getName()+ ";");
		...
		Task<Void> tsk= new Task<Void>() {
			...
			protected Void call() throws Exception 
			{
				...
				try 
				{
					...
					Dystrack.db.execRaw(ps); // write the statement down
	...
	public void addToHistory(long time)
	{
		try {
			...
			//			
			//			PreparedStatement psSel = Dystrack.db.getDb().prepareStatement(sqlSel);
			//			psSel.setString(1, song.getSongID());
			//			psSel.setLong(2, this.time);
			//			psSel.setString(3, vw.getUserID());
			//			
			//			ResultSet rsSel = Dystrack.db.execRaw(psSel);
			//			
			...
			PreparedStatement psIns = Dystrack.db.getDb().prepareStatement(sqlIns);
